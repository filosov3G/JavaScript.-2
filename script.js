...
class GoodsList {
  ...
  { title: 'Shoes', color: 'black', size: 23, brand: 'Nike', price: 250 },
	...
	goodsTotalPrice() {
		let totalPrice = this.goods.reduce((sum, current) => { return sum + current.price;
		}, 0);
...
class GoodsBasket {
	...
	goodsBasketTotalPrice() {
		let basketTotalPrice = this.goodsBasket.reduce((sum, current) => { return sum + current.price;
		}, 0);
